...
__global__ void gpu_unmix16_1(int16_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples, int32_t mixbits, int32_t mixres)
{
...
__global__ void gpu_unmix16_2(int16_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
...
void unmix16( int32_t * u, int32_t * v, int16_t * out, uint32_t stride, int32_t numSamples, int32_t mixbits, int32_t mixres )
{
	...
	{
		/* matrixed stereo */
		gpu_unmix16_1 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples, mixbits, mixres);
	}
	else
	{
		/* Conventional separated stereo. */
		gpu_unmix16_2 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples);
	}
}
...
__global__ void gpu_unmix24_1_1(uint8_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples, uint16_t * shiftUV, int32_t mixres, int32_t mixbits, int32_t shift)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
...
__global__ void gpu_unmix24_1_2(uint8_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples, int32_t mixres, int32_t mixbits)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
...
__global__ void gpu_unmix24_2_1(uint8_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples, uint16_t * shiftUV, int32_t shift)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
...
__global__ void gpu_unmix24_2_2(uint8_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
				...
				int32_t mixbits, int32_t mixres, uint16_t * shiftUV, int32_t bytesShifted )
{
	...
	if ( mixres != 0 )
	{
		...
		if ( bytesShifted != 0 )
		{
			...
			gpu_unmix24_1_1 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples, shiftUV, mixres, mixbits, shift);

		}
		else
		{
			gpu_unmix24_1_2 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples, mixres, mixbits);
		}
	}
	else 
	{
		...
		if ( bytesShifted != 0 )
		{
			...
			gpu_unmix24_2_1 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples, shiftUV, shift);
		}
		else
		{
			gpu_unmix24_2_2 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples);
		}
	}
...
__global__ void gpu_unmix32_1(int32_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples, uint16_t * shiftUV, int32_t mixres, int32_t mixbits, int32_t shift)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
...
__global__ void gpu_unmix32_2_1(int32_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
...
__global__ void gpu_unmix32_2_2(int32_t * op, uint32_t stride, int32_t * u, int32_t * v, int32_t numSamples, uint16_t * shiftUV, int32_t shift)
{
	int z = threadIdx.x + blockIdx.x * blockDim.x;
	if (z < numSamples)
	{
		...
		op += stride * z;
		op[0] = (u[z] << shift) | (uint32_t)shiftUV[k + 0];
		op[1] = (v[z] << shift) | (uint32_t)shiftUV[k + 1];
	}
}
				...
				int32_t mixbits, int32_t mixres, uint16_t * shiftUV, int32_t bytesShifted )
{
	...
	if ( mixres != 0 )
	{
		...
		gpu_unmix32_1 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples, shiftUV, mixres, mixbits, shift);
	}
	else
	{
		if ( bytesShifted == 0 )
		{
			...
			gpu_unmix32_2_1 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples);
		}
		else
		{
			/* interleaving with shift */

			gpu_unmix32_2_2 << < (numSamples + SIZE - 1) / SIZE, SIZE >> >(out, stride, u, v, numSamples, shiftUV, shift);
		}
	}
